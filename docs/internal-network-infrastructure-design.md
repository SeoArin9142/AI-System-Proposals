...
실제 운영에서는 아래 세 가지를 구분해서 설명하는 편이 좋습니다.

| 접속 모델 | 구조 | 장점 | 한계 | 권장도 |
|---|---|---|---|---|
| **완전 격리형** | 내부 전용 단말 또는 내부 전용 VDI만 사용 | 보안 논리가 가장 단순하고 설명이 명확함 | 사용자 편의성이 가장 낮고 도입 저항이 큼 | **보안 최우선 조직에 권장** |
| **회사 관리 PC + 내부 VDI** | 회사 관리 단말 -> 보안 게이트웨이 / VDI 브로커 -> 내부 VDI | 사용성과 통제가 균형적, 감사 정책 적용이 쉬움 | 완전 air-gap은 아님 | **통제형 내부망의 기본 권장안** |
| **개인 PC + 로컬 VM + 내부 VDI** | 개인 PC -> 개인 VM -> 보안 게이트웨이 / VDI 브로커 -> 내부 VDI | 화면 분리나 개인 사용성 측면에서는 편함 | 보안 핵심을 해결하지 못함. 개인 단말 자체의 키로거/캡처/계정 탈취/파일 유출 리스크는 그대로 남음 | **비권장, 예외적 허용만 검토** |

핵심 판단:

- **실제 개발 작업은 내부 VDI 안에서만 수행**해야 합니다.
- **로컬 VM은 보안 핵심이 아니라 편의용 분리 수단**에 가깝습니다.
- 따라서 `개인 PC + VM`보다 중요한 것은 아래입니다.
  - VDI 세션 통제
  - 클립보드 / 파일전송 / USB / 인쇄 제한
  - MFA
  - 세션 감사 로그
...
1. **최선**: 회사 관리 PC -> 내부 VDI
2. **차선**: 개인 PC -> VDI 클라이언트 -> 내부 VDI
3. **비권장**: 개인 PC 로컬 개발 후 일부만 내부 반입

따라서 "개발자는 외부망 PC를 쓰고, 실제 개발은 내부망 VDI에서만 수행"하는 구조는 충분히 현실적입니다.
다만 이 경우 문서상 분류는 **완전 격리형**이 아니라 **통제형 내부망 접속 모델**이어야 합니다.

### 7-4. VDI 운영 모델

직원 수가 늘어나도 **직원 1명당 GPU가 달린 전용 VDI 한 대**를 두는 방식으로 설계할 필요는 없습니다.

구분해야 하는 것은 아래 두 가지입니다.

- **사용자별 작업공간 분리**
- **AI / Git / Jenkins / 스토리지 같은 공용 자원 공유**

즉, 일반적으로는:

- **VDI 세션 또는 개발 작업공간은 개인별로 분리**
- **AI 서버와 Dev Tools 서버는 공용으로 운영**

운영 모델은 보통 아래 세 가지 중 하나를 고릅니다.

| 모델 | 구조 | 장점 | 단점 | 권장 상황 |
|---|---|---|---|---|
| **개인별 고정 VDI** | 사용자별로 고정된 VDI 할당 | IDE, 캐시, 로컬 설정, 작업 상태 유지가 쉬움 | 사용자 수만큼 자원 예약이 필요 | 소수 정예, 고정 개발 환경 선호 |
| **공용 VDI 풀 + 개인 저장공간** | 공용 VDI 풀에 접속하고 홈디렉토리/워크스페이스만 개인별 유지 | 자원 효율이 좋고 증설이 쉬움 | 이미지/프로파일 관리 체계가 필요 | **20~30명 규모의 기본 권장안** |
| **점프호스트 / 원격 IDE형** | GUI VDI 대신 원격 터미널 또는 웹 IDE 중심 | 가장 단순하고 비용이 낮음 | GUI 도구 사용성은 VDI보다 낮을 수 있음 | 텍스트 중심 개발, 최소 구성 |

실무 권장:

- **개발자별 계정과 작업공간은 분리**
- **GPU는 사용자별 전용이 아니라 AI 추론 서버에서 공용 사용**
- **VDI는 개인별 고정 또는 반고정 풀 방식으로 운영**

따라서 20~30명 개발 인력 기준에서는 아래 조합이 가장 현실적입니다.

1. **회사 관리 PC -> VDI 브로커 -> 공용 VDI 풀**
2. **개인별 홈디렉토리 / 개인별 workspace 유지**
3. **Git / Jenkins / AI / RAG는 공용 서버 사용**
4. **GPU는 중앙 AI 서버가 공유**

이 구조면 사용자 경험과 자원 효율을 같이 잡을 수 있습니다.
