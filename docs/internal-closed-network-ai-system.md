...
도입 목적:
- **보안성 확보**: 외부망과 차단된 내부 GPU/스토리지 환경에서만 코드와 로그를 다뤄 소스 유출 위험을 낮춤
- **업무 효율화**: Sparrow 결과, Jenkins 결과, 규정 검토 같은 반복 확인 업무를 AI가 1차 보조하여 개발자 검토 시간을 줄임
- **지식 자산화**: MISRA, CERT, Sparrow 대응 지식을 **규칙 카드(JSON)** 와 benchmark 형태로 정규화하여 조직 자산으로 축적
...
클라우드 동급 기준 참고값:
- AWS `p5.48xlarge` (8 x H100) 도쿄 리전 기준 **시간당 약 31.464 USD** 수준입니다.
...
외부망 선행 정리용 참고:
- Gemini 2.5 Pro API는 2026-05 확인 기준 **입력 1M tokens당 1.25 USD (<=200k), 출력 10 USD (<=200k)** 이며, 긴 컨텍스트에서는 단가가 더 올라갑니다.
...
|---|---|---:|---:|---:|
| Orchestrator | `p5.4xlarge` (1 x H100) | 약 **3.933 USD/h** | 약 **2,831.76 USD/월** | 약 **34,453.08 USD/년** |
| Owner AI | `p5.48xlarge` x 2 (총 16 x H100) | 약 **62.928 USD/h** | 약 **45,308.16 USD/월** | 약 **551,249.28 USD/년** |
| Reviewer AI | `p5.48xlarge` x 1 (8 x H100) | 약 **31.464 USD/h** | 약 **22,654.08 USD/월** | 약 **275,624.64 USD/년** |
| **합계** |  | **약 98.325 USD/h** | **약 70,794 USD/월** | **약 861,327 USD/년** |

환율 참고(2026-05 공개 환율 페이지 기준, **1 USD ≈ 1,456 KRW**)까지 단순 환산하면:
...
### 임원 보고용 ROI 논리
- **사후 수정비 절감**: 사람이 놓치기 쉬운 규정 위반과 보안 취약점을 AI가 먼저 걸러 재작업 비용을 낮춤
- **전문성 내재화**: 규칙 카드와 benchmark를 쌓는 과정 자체가 회사 고유의 리뷰 지식베이스가 됨
- **인력 교체 리스크 완화**: 특정 시니어 개인에게 몰린 리뷰 기준을 구조화 데이터로 남겨 조직 일관성을 높임
- **점진 확장 가능**: 처음에는 Reviewer 중심 MVP로 시작하고, 검증이 끝난 뒤 Owner 자동화 범위를 넓힐 수 있음
